...
tags: ["performance", "systems", "hardware"]
---

You swap two lines in a loop. Same algorithm. Same data. Runtime goes from 12ms to 620ms.

That's not a typo. **50x slower** from changing loop order.

Or: two threads increment separate variables. No shared state. No data race. Yet it runs 6x slower than it should.

These aren't edge cases. They're what happens when you ignore how CPUs actually access memory.

Your CPU runs at 4 GHz — 4 billion cycles per second. Each cycle takes 0.25 nanoseconds. Reading from RAM takes 100 nanoseconds. While waiting for one memory read, your CPU could execute 400 instructions. It just sits there. Waiting.

This is the **Memory Wall**. And everything in this article exists because of it.

## Why Does Memory Have Layers?

Modern CPUs don't just have "memory". They have a hierarchy — each level trading size for speed:

![CPU Memory Hierarchy](static/cpu-memory-hierarchy.svg)
...
## Why Can't You Read Just One Byte?
...
### Why Is Column Iteration 7x Slower?

C and C++ store 2D arrays in **row-major** order. Row 0 is contiguous in memory, then row 1, etc.
...
### Why Are Linked Lists 10x Slower Than Vectors?

Linked lists are taught in every CS course. They're also terrible for cache performance.
...
This is why Bjarne Stroustrup says: _"Vector is almost always the best choice."_

## Why Are My "Independent" Variables Slow?

This is where it gets interesting. And painful.

Two goroutines, each incrementing its own counter. No shared data. No locks. Should be embarrassingly parallel:
...
## How Do Cores Keep Their Caches Consistent?

When Core 1 writes to a variable, Core 2's cached copy becomes stale. How does the hardware handle this?
...
| State         | Meaning                                                                                  |
| ------------- | ---------------------------------------------------------------------------------------- |
| **Modified**  | I have the only copy. It's dirty (not in RAM). Must write back before anyone else reads. |
| **Exclusive** | I have the only copy. It matches RAM. Can modify without telling anyone.                 |
| **Shared**    | Multiple cores have this line. All match RAM. Must notify others before modifying.       |
| **Invalid**   | This cache line is garbage. Must fetch from another core or RAM.                         |

### State Transitions

![MESI State Transitions](static/mesi-state-transitions.svg)
...
## Why Are Memory Addresses Fake?

Every address your program uses is a lie.
...
![Page Table Translation](static/page-table-translation.svg)
...
![Multi-Level Page Tables](static/multi-level-page-table.svg)
...
## How Does the CPU Avoid 4 Memory Lookups Per Access?
...
The TLB holds 32-1024 entries depending on the CPU.
...
## Why Is Some RAM 50% Slower Than Other RAM?

On multi-socket servers, there's another layer of complexity.

**NUMA** = Non-Uniform Memory Access

![NUMA Architecture](static/numa-architecture.svg)
...
In SMP, all CPUs share one memory bus — which becomes the bottleneck as you add more cores. NUMA gives each socket its own memory controller, trading uniform latency for higher aggregate bandwidth.
...
When your program reads address X, here's the full journey:

![Memory Read Journey](static/memory-read-journey.svg)
...
- **ARM differences**: Apple M-series chips have 128-byte cache lines, not 64. The principles are the same, but padding sizes differ.
- **Benchmarks vary**: Numbers here are from an Intel i7. AMD, ARM, and different generations will show different ratios. The patterns hold; the exact multipliers don't.
- **Compiler optimizations**: Modern compilers can sometimes reorder struct fields or vectorize loops. `-O2` vs `-O0` can change whether cache effects are visible.
- **OS scheduling**: NUMA benchmarks assume the OS doesn't migrate your thread mid-run. In practice, it might.
- **Prefetcher limits**: Hardware prefetchers have stride limits. Very long strides or complex patterns won't be detected.

If you want to measure these effects on your own hardware, compile with `-O2 -fno-omit-frame-pointer` and use `perf stat -e cache-misses` to see real cache behavior.
